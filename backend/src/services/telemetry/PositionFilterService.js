...
class PositionFilterService {
  ...
  constructor({
    ...
    // { [deviceId]: { lastAccepted: {lat,lon,fixTime}, recentSpeedsKmh: number[], consecutiveRejects: number } }
    this.deviceState = {};
  }
   ...
   *
   * @param {Object} position - { deviceId, latitude, longitude, fixTime }
   * @returns {{ accepted: boolean, reason: string|null, impliedSpeedKmh: number|null,
   *   allowedMaxKmh: number|null, distanceMeters: number|null, resynced: boolean }}
   */
  evaluate(position) {
    const { deviceId, latitude, longitude } = position;
    const fixTime = position.fixTime instanceof Date ? position.fixTime : new Date(position.fixTime);
    ...
    const distanceMeters = haversineDistance(
      state.lastAccepted.lat, state.lastAccepted.lon, latitude, longitude
    );
    ...
    if (distanceMeters < this.jitterRadiusMeters) {
      this._accept(deviceId, { lat: latitude, lon: longitude, fixTime }, distanceMeters / dtSeconds * 3.6);
    ...
    if (state.consecutiveRejects >= this.maxConsecutiveRejects) {
      ...
      this._accept(deviceId, { lat: latitude, lon: longitude, fixTime }, impliedSpeedKmh, /* resync */ true);
      return this._result(true, 'resync_after_rejects', impliedSpeedKmh, allowedMaxKmh, distanceMeters, true);
    }

    return this._result(false, 'speed_jump', impliedSpeedKmh, allowedMaxKmh, distanceMeters, false);
  }

  _allowedMaxKmh(recentSpeedsKmh) {
  ...
  _accept(deviceId, point, speedKmh, resync = false) {
    if (!this.deviceState[deviceId]) {
      this.deviceState[deviceId] = { lastAccepted: null, recentSpeedsKmh: [], consecutiveRejects: 0 };
    }
    const state = this.deviceState[deviceId];
  ...
  _result(accepted, reason, impliedSpeedKmh, allowedMaxKmh, distanceMeters, resynced) {
    return { accepted, reason, impliedSpeedKmh, allowedMaxKmh, distanceMeters, resynced };
  }
}

module.exports = PositionFilterService;
