...
- **Automatización / provisión rápida** - pensado para instalar la app en una tableta nueva y que quede operando sola (ver también "QR de aprovisionamiento" más abajo, que además evita adb):
  - **Código de 4 dígitos "Modo Operador" y los valores base de NTRIP vienen del `.env` de la raíz del monorepo, no hardcodeados en el código** (`VITE_OPERATOR_MODE_CODE`/`VITE_NTRIP_DEFAULT_*`, ver sección 5 de `.env.example` - se leen en build time vía Vite, `envDir: '..'` en `web/vite.config.ts`, y quedan compilados dentro del bundle de la app) - al activarlo (`applyDefaultProvisioning()`) deja de un golpe: servidor de producción, envío continuo a 1s, NTRIP con esos valores base (nombre/host/puerto/versión/usuario/contraseña/mount point), baud rate 460800, salida SW Maps. El identificador de dispositivo y el token NUNCA se rellenan solos - varían por tableta, se quedan manuales a propósito; el mount point sí es configurable por `.env` ahora, pero queda vacío si no se define. El mismo botón "Restaurar valores por defecto" (Ajustes > Servicio GNSS) repite esto sin duplicar perfiles. Sin definir estas variables en el `.env`, cada campo cae a vacío (excepto el código, que cae a `3009`) y se llenan a mano como antes.
