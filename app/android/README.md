...
- `kiosk/KioskManager.kt` - una vez que la app YA es Device Owner, activa `Lock Task Mode` (bloquea recientes/inicio/barra de estado/notificaciones), registra la app como pantalla de inicio automática, y bloquea toda la pantalla de "Opciones de desarrollador" de Android (`DISALLOW_DEBUGGING_FEATURES` - no existe una API más fina para bloquear solo "ubicación falsa", esta es la única palanca real). **Configura el RTK/mock-location ANTES de activar el Modo Kiosko** - una vez activo, esa pantalla queda inaccesible hasta apagar el switch. A diferencia de liberar Device Owner, apagar el Modo Kiosko es reversible sin adb en cualquier momento (`exitKiosk` quita esa restricción).
...
7. Abre la app, entra a Ajustes (engranaje en el login), pon una contraseña de ajustes si no tiene una, y configura RTK/NTRIP (incluida "Seleccionar app de ubicación falsa" en Opciones de desarrollador) **antes** del siguiente paso - una vez activo el Modo Kiosko, esa pantalla de Android queda bloqueada.
8. Activa el switch "Modo Kiosko".
