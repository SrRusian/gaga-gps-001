...
const { toPublicShape } = require('./maps.routes');

const ALLOWED_IMAGE_EXT = ['.tif', '.tiff', '.jpg', '.jpeg'];
const ALLOWED_WORLD_EXT = ['.tfw', '.jpw', '.wld'];
const CRS_PATTERN = /^EPSG:\d+$/;

function buildMapsAdminRouter({ mapRepo, mapPipelineService, mapsDir, invalidateTilesCache, socketServer }) {
  ...
  const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
      ...
      cb(okImage || okWorld ? null : new Error(`Extensión no permitida para ${file.fieldname}: ${ext}`), okImage || okWorld);
    }
  });
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      console.error('❌ maps-admin.routes GET /:', err.message);
      res.status(500).json({ error: 'Error obteniendo mapas' });
    }
  });

  router.post('/', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'worldFile', maxCount: 1 }]), async (req, res) => {
    const imageFile = req.files?.image?.[0];
    const worldFile = req.files?.worldFile?.[0];
    ...
    try {
      ...
      console.error('❌ maps-admin.routes POST /:', err.message);
      if (!res.headersSent) res.status(500).json({ error: 'Error iniciando la importación del mapa' });
    }
  });
  ...
  router.patch('/:id', async (req, res) => {
    try {
      ...
      const map = await mapRepo.rename(req.params.id, name);
      if (!map) return res.status(404).json({ error: 'Mapa no encontrado' });
      res.json(map);
    } catch (err) {
      console.error('❌ maps-admin.routes PATCH /:id:', err.message);
  ...
  router.post('/:id/activate', async (req, res) => {
    try {
      const map = await mapRepo.findById(req.params.id);
      if (!map) return res.status(404).json({ error: 'Mapa no encontrado' });
      if (map.status !== 'ready') {
      ...
      console.error('❌ maps-admin.routes POST /:id/activate:', err.message);
  ...
  router.post('/:id/deactivate', async (req, res) => {
    try {
      ...
      console.error('❌ maps-admin.routes POST /:id/deactivate:', err.message);
  ...
  router.delete('/:id', async (req, res) => {
    try {
      ...
      if (!map) return res.status(404).json({ error: 'Mapa no encontrado' });
      if (map.active) {
        return res.status(409).json({ error: 'No se puede eliminar el mapa activo — desactívelo primero' });
      }

      await mapPipelineService.deleteFile(map.mbtiles_filename);
      invalidateTilesCache(map.id);
      await fs.rm(path.join(sourcesDir, String(map.id)), { recursive: true, force: true }).catch(() => {});
      await mapRepo.delete(map.id);

      res.json({ success: true });
    } catch (err) {
      console.error('❌ maps-admin.routes DELETE /:id:', err.message);
...
module.exports = buildMapsAdminRouter;
